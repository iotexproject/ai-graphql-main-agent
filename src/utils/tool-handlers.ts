...
export async function handleHttpRequest(params: {
  ...
  try {
    const { url, method, headers = {}, body, params: queryParams, env } = params;
    console.log('HTTP Request:', { url, method, headers, queryParams });
    ...
    // 生成缓存键
    const cacheKey = generateCacheKey(url, method, headers, body, queryParams);
